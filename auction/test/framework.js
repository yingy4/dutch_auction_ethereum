...
module.exports = function (accounts, done, schema) {
  ...
  DAuctions.deployed().then(function (instance) {
      ...
      function run_(block, index) {
        ...
        var promise;
        var account = web3.eth.accounts[action.account];
        console.log("Running account number: " + action.account + ",account address: "+ account);
        var nonce = nonces[action.account];
        switch (action.action) {
          case "bid":
            console.log("bid from: "+ account);
            promise = maincontract.bid(auctionId, { value: action.payment, from:account });
            break;
          case "finalize":
            console.log("finalize from: "+ account);
            promise = maincontract.finalize(auctionId, {from: account});
            break;
          case "refund":
            console.log("refund from: "+ account);
            promise = maincontract.refund(auctionId, action.amount, {from: account});
            break;
          default:
